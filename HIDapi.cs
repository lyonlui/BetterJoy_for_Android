	...
	public class HIDapi
	{
		...
		const string dll = "hidapi.dll";
			...
			public struct hid_device_info
			{
				...
				private IntPtr _serial_number;
				public ushort release_number;
				[MarshalAs(UnmanagedType.LPWStr)]
				public string manufacturer_string;
				private IntPtr _product_string;
				...
				public string serial_number
				{
					get {
						...
						for (var i = 0; i <=Marshal.SizeOf(_serial_number)*2; i++)
						{
							var _p = _serial_number + i * 4;
							var c = Marshal.ReadInt32(_p);
							if (c == 0) break;
							b.Append(char.ConvertFromUtf32(c));
						}
						return b.ToString().Replace(":",String.Empty);
			...
			public string product_string
			{
				get
				{
					...
					if (_product_string == IntPtr.Zero)
					{
						return string.Empty;
					}
					...
					for (var i = 0; i <= Marshal.SizeOf(_product_string) * 2; i++)
					{
						var _p = _product_string + i * 4;
						var c = Marshal.ReadInt32(_p);
						if (c == 0) break;
						b.Append(char.ConvertFromUtf32(c));
					}
