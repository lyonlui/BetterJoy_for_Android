...
namespace Joycon
{
    public class UdpServer
    {
		...
		public void Start(IPAddress ip, int port = 26760)
		{
			...
			{
				mActivity.console.Append("Motion server is OFF.\r\n");
				return;
			}
			...
			catch (SocketException ex)
			{
				...
				mActivity.console.Append("Could not start server. Make sure that only one instance of the program is running at a time and no other CemuHook applications are running.\r\n");
				return;
			}
			...
			running = true;
			mActivity.console.Append(String.Format("Starting server on {0}:{1}\r\n", ip.ToString(), port));
			StartReceive();
		}
