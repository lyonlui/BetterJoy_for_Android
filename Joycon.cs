...
namespace Joycon
{
	public class Joycon : Java.Lang.Object
	{
		float timing = 120.0f;
		...
		{
			this.activeData = mActivity.activeCaliData(serial_number);
		}
		...
		public int Attach(byte leds_ = 0x0)
		{
			...
			else
			{
				...
				mActivity.AppendTextBox("Using USB.\r\n");
		...
		{ // battery changed level
			...
			foreach (var v in mActivity.con)
			{
				if (v.Tag == (Object)this)
				{
					switch (battery)
					{
						case 4:
						...
						case 3:
							...
							break;
						case 2:
							v.SetBackgroundColor(Android.Graphics.Color.GreenYellow);
							break;
						case 1:
		...
		private int ReceiveRaw()
		{
			...
			if (ret > 0)
			{
				...
				{
					mActivity.AppendTextBox("Duplicate timestamp enqueued.\r\n");
		...
		*/
		...
		private void Poll()
		{
			...
			while (!stop_polling & state > state_.NO_JOYCONS)
			{
				...
				{
					state = state_.DROPPED;
					mActivity.AppendTextBox("Dropped.\r\n");
				...
				{
					// An error on read.
					//mActivity.AppendTextBox("Pause 5ms\n");
		...
		private int ProcessButtonsAndStick(byte[] report_buf)
		{
			...
			if (!isSnes)
			{
				...
				ushort[] cal = mActivity.nonOriginal ? new ushort[6] { 2048, 2048, 2048, 2048, 2048, 2048 } : stick_cal;
				ushort dz = mActivity.nonOriginal ? (ushort)200 : deadzone;
				...
				if (isPro)
				{
					...
					stick2 = CenterSticks(stick2_precal, mActivity.nonOriginal ? cal : stick2_cal, deadzone2);
				}
					...
					for (int i = 0; i < 3; ++i)
					{
						switch (i)
						{
							case 0:
								...
								if (mActivity.calibrate)
								{
									mActivity.xA.Add(acc_r[i]);
									mActivity.xG.Add(gyr_r[i]);
								}
								break;
							case 1:
								...
								if (mActivity.calibrate)
								{
									mActivity.yA.Add(acc_r[i]);
									mActivity.yG.Add(gyr_r[i]);
								}
								break;
							case 2:
								...
								if (mActivity.calibrate)
								{
									mActivity.zA.Add(acc_r[i]);
									mActivity.zG.Add(gyr_r[i]);
								}
								break;
						}
					}
		...
		public void Begin()
		{
			if (PollThreadObj == null)
			{
				...
				mActivity.AppendTextBox("Starting poll thread.\r\n");
			}
			else
			{
				mActivity.AppendTextBox("Poll cannot start.\r\n");
			}
		}
		...
		private void dump_calibration_data()
		{
			if (!isSnes)
			{
				...
				for (int i = 0; i < 9; ++i)
				{
					if (buf_[i] != 0xff)
					{
						mActivity.AppendTextBox("Using user stick calibration data.\r\n");
						found = true;
						break;
					}
				}
				if (!found)
				{
					mActivity.AppendTextBox("Using factory stick calibration data.\r\n");
